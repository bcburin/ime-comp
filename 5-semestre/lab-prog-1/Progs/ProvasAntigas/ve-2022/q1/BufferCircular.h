...
class BufferCircular {
  ...
  public:
    ...
    BufferCircular(const BufferCircular<T>& original, int tamanho_maximo);
    ~BufferCircular();
    T get(int index);
    int tamanho();
    bool cheio();
...
template <typename T>
BufferCircular<T>::BufferCircular(const BufferCircular<T>& original, int tamanho_maximo)
: tamanho_maximo(tamanho_maximo), inicio(original.inicio), fim(original.fim),
  buffer(new T[tamanho_maximo]) {
  // Copiar valores
...
template <typename T>
T BufferCircular<T>::get(int index) { 
  index = mod(inicio + index, tamanho_maximo);
  return buffer[index];
}
